...
# ZTE SKULL-THICKNESS EXTRACTION ALONG EEG TRAJECTORIES

This script estimates ZTE-derived skull thickness along EEG-normal trajectories.

Important:
    Skull thickness is anatomical and not frequency-dependent.
    The 250 kHz ZTE folder is used only as the source folder for locating
    each subject's ZTE image and anatomical directory structure.

Main workflow:
    1. Search ZTE subject folders from the 250 kHz source directory.
    2. Load each subject's ZTE image and EEG-normal CSV.
    3. Exclude predefined EEG locations.
    4. Sample ZTE intensity along each inward EEG-normal trajectory.
    5. Estimate skull thickness from raw ZTE high-intensity skull-profile peaks.
...
SEARCH_ROOTS = [
    {
        "modality": "ZTE",
        "frequency_khz": 250,
        "root": Path("/Volumes/Trans_2/transmission/data/ZTE/ZTE_f_250Hz"),
    },
]
...
OUT_SUBDIR = "skullthickness_ZTE_from_eeg_normals"
...
def build_skull_mask(zte_data: np.ndarray, threshold: float = ZTE_THRESHOLD) -> np.ndarray:
    mask = zte_data > threshold
...
def measure_skull_from_raw_zte_profile(
    distances: np.ndarray,
    raw_profile: np.ndarray,
):
    high_mask = (raw_profile >= CORTICAL_BONE_THRESHOLD).astype(np.uint8)
    ...
    if not high_info:
        ...
        diag = {
            ...
            "MeasurementSource": "RAW_ZTE_TWO_PEAK_SIMPLE",
        }
...
def find_zte_image(anat_dir: Path):
    candidates = sorted(
        p for p in anat_dir.glob("*_ZTE.nii.gz")
...
def run_folder(anat_dir: Path, modality: str, frequency_khz: int):
    ...
    image_path = find_zte_image(anat_dir)

    if image_path is None:
        print(f"SKIP (no ZTE image): {anat_dir}")
        return pd.DataFrame()
    ...
    skull_mask_3d = build_skull_mask(raw_data, threshold=ZTE_THRESHOLD)
    ...
    print(f"\nRUN ZTE skull thickness from 250 kHz folder  {subj}")
    ...
    for _, row in eeg_df.iterrows():
        ...
        try:
            ...
            chosen_seg, high_mask_raw, skull_span_mask, selection_note, diag = measure_skull_from_raw_zte_profile(
                distances,
                raw_profile,
            )
            ...
            axes[2].set_ylabel("Raw ZTE > threshold")
            ...
            records.append({
                ...
                "CorticalBoneThreshold": CORTICAL_BONE_THRESHOLD,
                ...
                "Diagnostic3DMaskThreshold": ZTE_THRESHOLD,
...
def main():
    ...
    if all_records:
        ...
        master_out_dir = Path("/Volumes/Trans_2/transmission/data/ZTE")
        master_out_dir.mkdir(parents=True, exist_ok=True)

        out_master = master_out_dir / "ZTE_skull_thickness_summary_250kHz_from_eeg_normals.csv"
        ...
        out_thin_master = master_out_dir / "ZTE_skull_thickness_less_than_2p5mm_250kHz_from_eeg_normals.csv"
        ...
        out_two_peak_master = master_out_dir / "ZTE_two_peak_span_rows_250kHz_from_eeg_normals.csv"
        ...
        out_flagged_master = master_out_dir / "ZTE_flagged_QC_rows_250kHz_from_eeg_normals.csv"
